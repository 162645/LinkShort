...
func (h *AnalyticsHandler) GetURLStats(ctx context.Context, req *pb.StatsRequest, rsp *pb.StatsResponse) error {
	...
	// Map the comprehensive stats report into the protobuf response.
	...
	// Time series points
	for _, ts := range statsReport.TimeSeries {
		rsp.TimeSeries = append(rsp.TimeSeries, &pb.TimeSeriesPoint{
	...
	// Country stats
	for _, country := range statsReport.CountryStats {
		rsp.CountryStats = append(rsp.CountryStats, &pb.CountryStats{
			Country:    country.Country,
			Clicks:     country.Clicks,
			Percentage: float32(country.Percentage),
		})
	}

	// Device stats
	for _, device := range statsReport.DeviceStats {
		rsp.DeviceStats = append(rsp.DeviceStats, &pb.DeviceStats{
			DeviceType: device.DeviceType,
			Clicks:     device.Clicks,
			Percentage: float32(device.Percentage),
		})
	}

	// Browser stats
	for _, browser := range statsReport.BrowserStats {
		rsp.BrowserStats = append(rsp.BrowserStats, &pb.BrowserStats{
			Browser:    browser.Browser,
			Clicks:     browser.Clicks,
			Percentage: float32(browser.Percentage),
		})
	}

	// Referrer stats
	for _, referrer := range statsReport.ReferrerStats {
		rsp.ReferrerStats = append(rsp.ReferrerStats, &pb.ReferrerStats{
			Referrer:   referrer.Referrer,
			Clicks:     referrer.Clicks,
			Percentage: float32(referrer.Percentage),
		})
	}
	...
	h.log.WithFields(logrus.Fields{
		...
		"time_series":    len(rsp.TimeSeries),
		"country_stats":  len(rsp.CountryStats),
		"device_stats":   len(rsp.DeviceStats),
		"browser_stats":  len(rsp.BrowserStats),
		"referrer_stats": len(rsp.ReferrerStats),
	}).Info("URL statistics retrieved successfully with full breakdown")
...
func (h *AnalyticsHandler) GetDashboard(ctx context.Context, req *pb.DashboardRequest, rsp *pb.DashboardResponse) error {
	...
	// Map dashboard metrics into the protobuf response.
	...
	// Click timeline
	for _, ts := range dashboard.ClickTimeline {
		rsp.ClickTimeline = append(rsp.ClickTimeline, &pb.TimeSeriesPoint{
			Timestamp:    ts.Timestamp.Unix(),
			Clicks:       ts.Clicks,
			UniqueClicks: ts.UniqueClicks,
		})
	...
	// Top countries
	for _, country := range dashboard.TopCountries {
		rsp.TopCountries = append(rsp.TopCountries, &pb.CountryStats{
			Country:    country.Country,
			Clicks:     country.Clicks,
			Percentage: float32(country.Percentage),
		})
	...
	// Device breakdown
	for _, device := range dashboard.DeviceBreakdown {
		rsp.DeviceBreakdown = append(rsp.DeviceBreakdown, &pb.DeviceStats{
			DeviceType: device.DeviceType,
			Clicks:     device.Clicks,
			Percentage: float32(device.Percentage),
		})
	...
	// NOTE: domain.DashboardMetrics does not expose a "top URLs" collection,
	// so rsp.TopUrls has no data source and is left empty.
	...
	h.log.WithFields(logrus.Fields{
		...
		"click_timeline":   len(rsp.ClickTimeline),
		"top_countries":    len(rsp.TopCountries),
		"device_breakdown": len(rsp.DeviceBreakdown),
	}).Info("Dashboard metrics returned successfully with full breakdown")
